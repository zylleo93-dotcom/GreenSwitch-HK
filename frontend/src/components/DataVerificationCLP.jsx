...
import { CheckCircle, Zap, Plus, Trash2, ChevronRight, X, Upload, FileText, Camera, DollarSign, Loader2 } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import FileUpload from './FileUpload.jsx';
import { analyzeImages } from '../utils/aiService.js';

const DataVerificationCLP = ({ data, onDataChange, onConfirm, globalFiles, onGlobalFileUpload }) => {
  ...
  const handleDeviceFileUpload = (rowId, files) => {
    if (!files) return;
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex">
            <div className="w-screen max-w-md bg-white shadow-2xl flex flex-col">
              <div className="px-6 py-6 border-b border-slate-100 flex items-center justify-between">
                <div>
                  <div className="flex items-center space-x-2">
                    <h2 className="text-xl font-black text-slate-800 tracking-tight">設備文件管理</h2>
                    {editingItem?.isAiGenerated && (
                  ...
                  <p className="text-xs text-slate-400 mt-1 font-medium">項目編號: <span className="text-amber-600 font-bold">{editingItem?.projectId || '---'}</span></p>
                </div>
                <button onClick={() => setDrawerOpen(false)} className="p-2 hover:bg-slate-50 rounded-full transition-colors">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-8">
                <section>
                  <div className="flex items-center justify-between mb-4">
                    ...
                    <button 
                      ...
                      disabled={isAnalyzing || !deviceFiles[editingRowId] || deviceFiles[editingRowId].length === 0}
                      className={`px-3 py-1.5 rounded-lg text-[10px] font-black flex items-center space-x-1.5 transition-all ${
                        isAnalyzing || !deviceFiles[editingRowId] || deviceFiles[editingRowId].length === 0
                          ? 'bg-slate-100 text-slate-400 cursor-not-allowed'
                  ...
                  <div className="grid grid-cols-1 gap-4">
                    <div 
                      ...
                      className="p-8 border-2 border-dashed border-slate-200 rounded-2xl bg-slate-50/50 hover:bg-slate-50 hover:border-amber-300 transition-all cursor-pointer group"
                    >
                      <input type="file" ref={fileInputRef} className="hidden" onChange={(e) => handleDeviceFileUpload(editingRowId, e.target.files)} multiple />
                      <div className="flex flex-col items-center">
                        ...
                        <span className="text-xs font-black text-amber-600 uppercase tracking-wider">上傳此設備文件</span>
                      </div>
                    </div>
                          ...
                          <div key={idx} className="relative group aspect-video">
                            <div className="w-full h-full bg-white border border-slate-200 rounded-xl flex items-center justify-center overflow-hidden shadow-sm">
                              {file.file?.type?.startsWith('image/') || file.type?.startsWith('image/') ? (
                                <img src={file.preview || URL.createObjectURL(file)} alt="preview" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                              ) : (
                ...
                <section>
                  ...
                  <div className="space-y-2">
                    {[
                      { label: '電費單據', count: globalFiles.electricity.length, color: 'bg-blue-50 text-blue-600' },
                      { label: '新設備能源標籤', count: globalFiles.efficiency.length, color: 'bg-amber-50 text-amber-600' },
                      { label: '舊設備能源標籤', count: globalFiles.oldEfficiency.length, color: 'bg-emerald-50 text-emerald-600' },
                      { label: '財務證明', count: globalFiles.financial.length, color: 'bg-purple-50 text-purple-600' }
                    ].map((type, idx) => (
