...
import { TrendingUp, Leaf, Award, ChevronLeft, Zap, ArrowRight, ShieldCheck, ChevronDown, ExternalLink, ChevronRight, LogIn, LogOut, User } from 'lucide-react';
...
const Index = () => {
  ...
  const handleFileUploadCLP = (type, files) => {
  ...
  const handleFileUploadHEC = (type, files) => {
    const fileArray = Array.isArray(files) ? files : (files ? [files] : []);
              ...
              {user ? (
                <div className="flex items-center space-x-3">
                  <div className="flex items-center space-x-2 px-3 py-1.5 bg-slate-50 rounded-full border border-slate-100">
                    ...
                    <span className="text-xs font-medium text-slate-700 max-w-[100px] truncate">{user.displayName || user.email}</span>
                  </div>
                  <button 
                ...
                <button
                  onClick={handleLogin}
                  className="flex items-center space-x-2 px-4 py-2 bg-slate-900 text-white rounded-xl text-sm font-bold hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10"
                >
                  <LogIn className="w-4 h-4" />
                  <span>登入以啟用 AI 緩存</span>
                </button>
              )}
            </div>
          </div>
