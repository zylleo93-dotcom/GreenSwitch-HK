...
import { DollarSign, Leaf, Clock, Award, Info, ExternalLink, Eye, LayoutDashboard } from 'lucide-react';
...
const Step4CLP = ({ 
  ...
  const [viewMode, setViewMode] = useState('dashboard'); // 'dashboard' | 'preview'
  ...
  return (
    <div className="space-y-8">
        ...
        <div className="flex bg-slate-100 p-1 rounded-xl self-start">
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center space-x-2 transition-all ${
              viewMode === 'dashboard' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
          ...
          <button 
            ...
            className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center space-x-2 transition-all ${
              viewMode === 'preview' ? 'bg-white text-amber-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'
            }`}
          >
      ...
      <SensitivityAnalysis
        ...
        baseElectricityPrice={reportData.items?.[0]?.electricityPrice || 1.2}
      />
      ...
      <ReportPreview data={reportData} utility="CLP" />
    )}
  </div>
);
};
