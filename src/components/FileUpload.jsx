  ...
  const onDrop = useCallback(
    (acceptedFiles) => {
      if (acceptedFiles.length > 0) {
        const newFilesList = multiple ? [...files, ...acceptedFiles] : [acceptedFiles[0]];
        
        // Limit number of files if needed
        const finalFiles = multiple && maxFiles ? newFilesList.slice(0, maxFiles) : newFilesList;
        
        onFileUpload(multiple ? finalFiles : finalFiles[0]);
      }
    },
    [onFileUpload, files, multiple, maxFiles]
  );

  const removeFile = (e, fileToRemove) => {
    e.stopPropagation();
    const newFilesList = files.filter(f => f !== fileToRemove);
    ...
    onFileUpload(multiple ? newFilesList : (newFilesList[0] || null));
  };
