...
export const calculateEnergySavings = (item) => {
  ...
  if (item.utility === 'HEC') {
    ...
    if (pDate < startDate || pDate > endDate) {
      ...
      const eightyPercentTotalCost = totalCost * 0.8;

      if (item.deviceType === 'LED_BULB' || item.deviceType === 'LED_DOWNLIGHT') {
        // LED 燈膽/筒燈: 每件 $20 或 單價 80%
        estimatedSubsidy = Math.min(20 * item.quantity, eightyPercentTotalCost);
      } else if (item.deviceType === 'LED_TUBE' || item.deviceType === 'LED_STRIP' || item.deviceType === 'LED_PANEL' || item.deviceType === 'LED_LIGHTING') {
        // LED 光管/燈帶/燈盤: 每件 $60 或 單價 80%
        estimatedSubsidy = Math.min(60 * item.quantity, eightyPercentTotalCost);
      } else if (item.deviceType === 'AC_GRADE1') {
        // 一級冷氣機: 每台 $1,000 或 單價 80%
        estimatedSubsidy = Math.min(1000 * item.quantity, eightyPercentTotalCost);
      } else {
        // 其他節能裝置: 按節電量計算，每度電 (kWh) $0.5
        // 雖然圖片沒寫 80% 限制，但通常補貼不超過成本，這裡保留 totalCost 限制
        estimatedSubsidy = Math.min(annualSavingsKWh * 0.5, totalCost);
      }
    }
  }
...
export const calculateBatchSavings = (data) => {
  const { items = [], utility, hasEnergyAudit } = data;
  const globalElectricityPrice = data.electricityPrice;
  
  if (items.length === 0) return null;

  // 對每個項目單獨計算
  const itemResults = items.map(item => {
    // 優先使用項目自身的電價，如果沒有則使用全局電價，最後使用默認值 1.2
    const price = item.electricityPrice || globalElectricityPrice || 1.2;
